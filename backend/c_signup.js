...
module.exports = async function(req,res,connection){
    try{
        ...
        req.on("end", async function(){
            ...
            await connection.query(query1, params1, (err) => {
                if(err) throw err;

                res.end(JSON.stringify({message: "Customer's information updated successful"}));
            });
            ...
            await connection.query(query2, params2, (err) => {
                if(err) throw err;

                res.end(JSON.stringify({message: "Address information updated successful"}));
            });
            ...
            await connection.query(query3, params3, (err) => {
                if(err) throw err;

                res.end(JSON.stringify({message: "All information updated successful"}));
            });
        });
    }
    catch(error){
