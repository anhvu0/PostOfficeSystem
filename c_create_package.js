...
module.exports = function (req,res,connection,customerId){
    try{
        ...
        req.on("end", async function(){
            ...
            let params;
            let result;
            if(!parsedData.weight || !parsedData.street_address_to || !parsedData.city_to || !parsedData.state_to || !parsedData.zip_to || !parsedData.location_name
                || !parsedData.receive_f_name || !parsedData.receive_l_name){
            ...
            query = 'SELECT address_id FROM customers WHERE customers_id = ?';
            params = [customerId];
            result = await executeQuery(connection, query, params);
            address_from_id = result[0].address_id;

            // exists = true;      DOESNT USE YET BECAUSE THE ADDRESS IS ALREADY LINKED WITH THE CUSTOMER
            // while(exists) {
